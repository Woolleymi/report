// WMI Lifting Inspection – App (UMD libs, no imports)
(function(){
  const todayStr = () => new Date().toISOString().slice(0,10);
  const uid = () => Math.random().toString(36).slice(2, 9);
  const yearOf = (iso) => (iso ? new Date(iso).getFullYear() : new Date().getFullYear());
  const seqKey = (k, yr) => `${k}_${yr}`;
  const getNextSeq = (k, yr) => { try { const key = seqKey(k, yr); const n = parseInt(localStorage.getItem(key) || "0", 10) + 1; localStorage.setItem(key, String(n)); return n; } catch { return Math.floor(Math.random()*9000)+1000; } };
  const pad4 = (n) => String(n).padStart(4,"0");

  const DEFAULT_LOCATIONS = ["Main Roof","HVAC 1","HVAC 2","Plant Floor","Warehouse A","Warehouse B","Loading Dock","Exterior Yard"];
  const EQUIPMENT_TYPES = ["Anchor Point","Fall Arrest Lanyard","Overhead Crane","Gantry Crane","Jib Crane","Forklift Attachment","Chain Hoist","Lever Hoist","Slings – Webbing","Slings – Chain","Lift Table","Other"];

  const COMPANY = { name: "Woolley's Mill Inspection Ltd.", address1: "45 Thomson Rd.", address2: "Simcoe, ON  N3Y 4Z6", phone: "905-906-4591", email: "adam@wmiltd.ca" };

  function useLocalStorage(key, init) {
    const [state, setState] = React.useState(() => { try { const raw = localStorage.getItem(key); return raw ? JSON.parse(raw) : init; } catch { return init; } });
    ...
    return `
      <div style="position:relative;width:780px;border-radius:12px;font-family:ui-sans-serif,system-ui,-apple-system;">
        ...
        <div style="margin-top:16px;display:grid;grid-template-columns:1fr 1fr;gap:12px;font-size:13px;">
          ${row("Customer", entry.customer)} ${row("Contact Person", entry.contact)} ${row("Address", entry.address, 2)}
          ${row("Client Order No.", entry.clientOrderNo)} ${row("Our Inspection No.", entry.ourInspectionNo)}
        </div>
        <div style="margin-top:16px;display:grid;grid-template-columns:1fr 1fr;gap:12px;font-size:13px;">
          ${row("Lifting Device Make and Type", entry.makeAndType)} ${row("Capacity", entry.capacity)} ${row("Serial No.", entry.serialNo)} ${row("Location", entry.location)}
        </div>
        ...
        <div style="margin-top:12px;display:grid;grid-template-columns:1fr 1fr;gap:16px;">
          ...
          <div style="font-size:13px;">
            ...
            <div style="display:flex;gap:24px;align-items:center;margin:6px 0;">
              ${checkMark(entry.certified === 'Yes', 'Yes')} ${checkMark(entry.certified === 'No', 'No')}
            </div>
        ...
        <div style="margin-top:16px;font-size:11px;color:#6b7280;">INSPECTION: Pursuant to the current regulations of the Occupational Health and Safety Act dealing with lifting devices, an inspection must be performed at least once per year by a competent person. It is recommended that maintenance be done in accordance with the manufacturer's manual.</div>
        <div style="margin-top:8px;font-size:11px;color:#6b7280;">This certificate or report is valid only for the work which it specifically describes and is not transferable without written approval by Woolley's Mill Inspection Inc.</div>
      </div>`;
  }
  function row(k, v, span){ return `<div style="${span===2?'grid-column:1 / -1;':''}"><div style="display:flex;gap:8px;"><div style="width:240px;color:#6b7280;">${k}:</div><div style="font-weight:500;">${v || '—'}</div></div></div>`; }
  function scopeLine(label, tick){ return `<li style="display:flex;align-items:center;gap:8px;"><span style="display:inline-block;width:14px;height:14px;border:1px solid #111;background:${tick?'#111':'#fff'}"></span><span>${label}</span></li>`; }
  function checkMark(checked, label){ return `<span style="display:inline-flex;align-items:center;gap:8px;"><span style="display:inline-block;width:14px;height:14px;border:1px solid #111;background:${checked?'#111':'#fff'}"></span>${label}</span>`; }

  function useFormInit(){ return { customer:"", contact:"", address:"", clientOrderNo:"", ourInspectionNo:"", makeAndType:"", reportNo:"", capacity:"", serialNo:"", location:"", scopeA_visual:false, scopeB_loadTest:false, scopeC_mt:false, scopeD_utPins:false, scopeE_corrective:false, scopeF_other:false, scopeOtherText:"", certified:"Yes", comments:"", date:todayStr(), inspectorName:"", passFail:"Pass", newThisYear:false, missing:false, photoDataUrl:"" }; }

  function Section({ title, children }){
    return React.createElement("div", {style:{ border:"1px solid #e5e7eb", borderRadius:12, padding:16, marginTop:16 }},
      React.createElement("div", {style:{ fontWeight:600, marginBottom:8 }}, title), children);
  }
  ...
  function Field({ label, children, full, required }){
    return React.createElement("div", {style:{ gridColumn: full?"1 / -1":"auto" }},
      React.createElement("label", null, label, " ", required ? React.createElement("span", {style:{ color:"#dc2626" }}, "(required)") : null), children);
  }
  function Check({ line, checked, onChange }){
    return React.createElement("label", {style:{
        display:"flex", alignItems:"flex-start", gap:10, fontSize:14, cursor:"pointer",
        border:"1px solid #e5e7eb", padding:10, borderRadius:12, width:"100%", boxSizing:"border-box"
      }},
      React.createElement("input", {type:"checkbox", checked:!!checked, onChange:(e)=>onChange && onChange(e.target.checked), style:{marginTop:2}}),
      React.createElement("span", {style:{ flex:"1 1 auto", whiteSpace:"normal", wordBreak:"break-word", overflowWrap:"anywhere", lineHeight:1.25 }}, line)
    );
  }
  ...
  function App(){
    ...
    const requiredMissing = React.useMemo(()=>{
      const miss=[]; if(!form.customer.trim())miss.push("Customer"); if(!form.makeAndType.trim())miss.push("Make & Type");
      if(!form.serialNo.trim())miss.push("Serial No."); if(!form.location.trim())miss.push("Location"); if(!form.date.trim())miss.push("Date"); return miss;
    },[form]);
    const valid = requiredMissing.length===0;

    const summary = React.useMemo(()=>{
      const total=entries.length, pass=entries.filter(e=>e.passFail==="Pass").length, fail=entries.filter(e=>e.passFail==="Fail").length, missing=entries.filter(e=>e.missing).length;
      return { total, pass, fail, missing };
    },[entries]);

    const ensureNumbers = (f) => {
      const yr = yearOf(f.date); const next = { ...f };
      if(!next.reportNo){ const n=getNextSeq("report", yr); next.reportNo = `WMI-LD-${yr}-${pad4(n)}`; }
      if(!next.ourInspectionNo){ const n2=getNextSeq("inspection", yr); next.ourInspectionNo = `WMI-INS-${yr}-${pad4(n2)}`; }
      return next;
    };
    ...
    function addEntry(){
      ...
      setForm(useFormInit()); setPhotoName("");
      setTimeout(()=>setSaving(false), 300);
    }
    function deleteEntry(id){ setEntries(prev=>prev.filter(e=>e._id!==id)); }
    ...
    async function exportMasterXlsx(){
      ...
      try {
        ...
        const rows = entries.map(e => ({
          "Customer": e.customer, "Contact Person": e.contact, "Address": e.address, "Client Order No.": e.clientOrderNo,
          "Our Inspection No.": e.ourInspectionNo, "Make & Type": e.makeAndType, "Report No.": e.reportNo,
          "Capacity": e.capacity, "Serial No.": e.serialNo, "Location": e.location, "Certified": e.certified,
          "Comments": e.comments, "Date": e.date, "Inspector": e.inspectorName, "Pass/Fail": e.passFail,
          "New This Year": e.newThisYear ? "Yes":"No", "Missing": e.missing ? "Yes":"No"
        }));
        const wb = XLSX.utils.book_new();
        XLSX.utils.book_append_sheet(wb, XLSX.utils.json_to_sheet(rows), "2025 Inspection List");
        const roll = { Pass: entries.filter(e=>e.passFail==='Pass').length, Fail: entries.filter(e=>e.passFail==='Fail').length, Missing: entries.filter(e=>e.missing).length, Total: entries.length };
        XLSX.utils.book_append_sheet(wb, XLSX.utils.json_to_sheet([roll]), "Roll-up Counts");
        const syncRows = entries.map(e=>({ "Equipment Type": e.makeAndType, "ID Number": e.serialNo, "Pass/Fail": e.passFail, "New This Year": e.newThisYear? "Yes":"No", "Missing": e.missing? "Yes":"No", "Notes": e.comments, "Location": e.location, "Date": e.date }));
        XLSX.utils.book_append_sheet(wb, XLSX.utils.json_to_sheet(syncRows), "Sync – Device List");
        XLSX.writeFile(wb, `WMI_Master_Lifting_List_${todayStr()}.xlsx`);
      } finally { setBusy(false); }
    }

    async function html2canvasFromEntry(entry){
      const frag = document.createElement("div");
      frag.style.width = "780px"; frag.style.padding = "24px"; frag.style.border = "1px solid #e5e7eb";
      ...
      const canvas = await window.html2canvas(frag, { scale: 2 });
    ...
    async function exportSinglePdf(entry){
      ...
      try {
        ...
        const canvas = await html2canvasFromEntry(entry);
    ...
    return React.createElement("div", {style:{ maxWidth:880, margin:"0 auto", padding:16, paddingBottom:96, fontFamily:"ui-sans-serif, system-ui, -apple-system" }},
      React.createElement("div", {style:{ position:"sticky", top:0, zIndex:10, background:"rgba(255,255,255,0.85)", backdropFilter:"saturate(180%) blur(4px)", borderBottom:"1px solid #e5e7eb" }},
        React.createElement("div", {style:{ display:"flex", alignItems:"center", justifyContent:"space-between", padding:"12px 0" }},
          React.createElement("div", {style:{ fontSize:18, fontWeight:600 }}, "WMI – Lifting Device Inspection (PWA)"),
      ...
      React.createElement("div", {style:{ marginTop:24, display:"grid", gap:12 }},
        ...entries.map(entry => React.createElement("div", {key:entry._id, style:{ border:"1px solid #e5e7eb", borderRadius:12, padding:12 }},
          ...
          React.createElement("div", {style:{ marginTop:8, display:"flex", gap:8, alignItems:"center" }},
            ...
            React.createElement("span", {style:{ fontSize:11, background:"#f3f4f6", padding:"2px 8px", borderRadius:9999 }}, "Certified: ", entry.certified),
